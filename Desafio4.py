...
def obter_caminho_valido():
    while True:
        ...
        if os.path.isdir(folder_path):
            return folder_path
        print("Caminho inválido. Certifique-se de que o caminho está correto e tente novamente.")


def obter_formato_valido():
    while True:
        save_format = input("Deseja salvar o resultado em TXT ou Excel? (Digite 'TXT' ou 'Excel'): ").strip().lower()
        if save_format in ['txt', 'excel']:
            return save_format
        print("Opção inválida. Escolha 'TXT' ou 'Excel'.")


def process_excel():
    folder_path = obter_caminho_valido()
    save_format = obter_formato_valido()
