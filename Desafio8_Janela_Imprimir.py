...
def imprimir_pdf(file_path):
    """Abre o PDF no visualizador padrão e permite imprimir."""
    abrir_pdf(file_path)
    # O usuário pode usar a opção de impressão do visualizador de PDF.
    ...
    for file_name in arquivos_encontrados:
        ...
        try:
            ...
            if save_format == 'txt':
                ...
                return pdf_file_path  # Retorna o caminho do PDF para impressão
...
class App:
    def __init__(self, root):
        ...
        self.pdf_file_path = None  # Armazena o caminho do PDF gerado
        ...
        # Botão de impressão (visível apenas após gerar um PDF)
        self.btn_imprimir = ttk.Button(frame, text="Imprimir PDF", command=self.imprimir, state=tk.DISABLED)
        self.btn_imprimir.grid(row=7, column=0, columnspan=2, pady=10)
    ...
    def processar(self):
        ...
        try:
            self.pdf_file_path = process_excel(folder_path, save_format)
            if self.pdf_file_path:
                self.btn_imprimir.config(state=tk.NORMAL)  # Habilita o botão de impressão
        except Exception as e:
            messagebox.showerror("Erro", f"Ocorreu um erro: {str(e)}")

    def imprimir(self):
        if self.pdf_file_path and os.path.exists(self.pdf_file_path):
            imprimir_pdf(self.pdf_file_path)
        else:
            messagebox.showerror("Erro", "Nenhum arquivo PDF gerado para imprimir.")
